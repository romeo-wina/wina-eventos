...
export default function Home() {
  ...
  const descargarPDF = () => {
    const doc = new jsPDF();
    doc.text("WINA EVENTOS",105,15,{align:"center"});
    const body=[];
    resumenProductos.forEach(cat=>{body.push([{content:cat.nombre.toUpperCase(),colSpan:3}]);cat.items.forEach(item=>body.push([item.nombre,String(item.qty),"$"+item.subtotal.toLocaleString("es-AR")]))});
    autoTable(doc,{startY:25,head:[["Artículo","Cant.","Subtotal"]],body});
    doc.save("Presupuesto-Wina-Eventos.pdf");
  };
        ...
        <div className="wina-overlay" onClick={() => setVerPresupuesto(false)}>
          <div className="wina-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <button className="wina-btn-pdf" onClick={descargarPDF}>
